  ...
  return (
    <ToolShell title="Buffer Overflow Animator" description="Type characters into an 8-byte buffer. Watch what happens when you write past the end.">
      <div className="space-y-4">
        <div>
          <label className="block text-xs text-slate-600 mb-1">Input string (buffer size: {bufferSize} bytes)</label>
          <input
            type="text"
            value={input}
        ...
        <div className="border border-slate-300 rounded-lg overflow-hidden font-mono text-xs">
          ...
          {cells.map((cell) => (
            <div
              key={cell.addr}
              className={`px-3 py-1.5 flex items-center gap-3 border-b border-slate-100 ${
            ...
            >
              ...
              <span className={`font-bold ${cell.overflow ? "text-red-600" : cell.type === "ret" ? "text-red-400" : cell.type === "ebp" ? "text-amber-600" : "text-slate-600"}`}>
