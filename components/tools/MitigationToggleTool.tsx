      ...
      <div className="space-y-4">
        <div className="p-4 rounded-lg bg-surface-2 border border-subtle">
          <div className="flex justify-between items-center mb-2">
            <p className="text-sm font-medium text-secondary">Attack Feasibility Score</p>
            <p className={`text-2xl font-bold ${getScoreColor()}`}>{score}%</p>
          </div>
          <div className="w-full bg-strong rounded-full h-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <button onClick={() => toggle(m.id)} className={`w-10 h-5 rounded-full relative transition-colors ${enabled.has(m.id) ? "bg-emerald-500" : "bg-strong"}`}>
                    <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-surface-2 transition-all ${enabled.has(m.id) ? "left-5" : "left-0.5"}`} />
                  </button>
