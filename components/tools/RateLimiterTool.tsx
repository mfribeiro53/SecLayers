    ...
    <ToolShell title="Token Bucket Simulator" description="Visualize the token bucket algorithm in real time. Requests arrive randomly; tokens refill at a fixed rate.">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="block text-xs text-slate-600 mb-0.5">Refill rate (tokens/sec)</label>
            <input type="number" value={rate} onChange={(e) => setRate(Math.max(1, parseInt(e.target.value) || 1))} className="w-full px-2 py-1.5 border border-slate-300 rounded text-xs" />
          </div>
          <div>
            <label className="block text-xs text-slate-600 mb-0.5">Bucket capacity</label>
            <input type="number" value={capacity} onChange={(e) => setCapacity(Math.max(1, parseInt(e.target.value) || 1))} className="w-full px-2 py-1.5 border border-slate-300 rounded text-xs" />
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg border border-slate-200 bg-slate-50">
          <p className="text-sm font-medium text-slate-700">Tokens: {tokens.toFixed(1)} / {capacity}</p>
