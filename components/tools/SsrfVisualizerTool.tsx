  ...
  return (
    <ToolShell title="SSRF Visualizer" description="Enter a URL and see whether it would be allowed or blocked by SSRF defenses.">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-secondary mb-1">Target URL</label>
          <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => e.key === "Enter" && checkUrl()} placeholder="http://169.254.169.254/latest/meta-data/" className="w-full px-3 py-2 border border-subtle rounded-md font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          {targets.map((t) => (
            <button key={t.label} onClick={() => { setUrl(t.url); setResult(null); }} className="px-2 py-1 text-xs rounded bg-elevated text-secondary hover:bg-strong">{t.label}</button>
          ))}
        </div>
        {result && (
